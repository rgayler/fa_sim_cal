...
The snapshot ZIP file (682 MB compressed, 8.8 GB uncompressed) was
manually downloaded, preprocessed to fix issues with encoding nd
embedded nulls, then re-compressed in [XZ
format](https://en.wikipedia.org/wiki/XZ_Utils) to minimise the size
(332 MB) for storage and publication. The compressed snapshot file and
the data dictionary file are stored in the `data/` directory.

The preprocessing steps take the downloaded ZIP file as input and were
carried out as linux shell commands. The relevant code chunks below are
not evaluated because the distributed version of this project will *not*
...
The data dictionary indicates that the data is
[encoded](https://en.wikipedia.org/wiki/Character_encoding) as
[UTF-16LE](https://en.wikipedia.org/wiki/UTF-16#Byte-order_encoding_schemes)
I don't have any details on how the this data is collected at source and
...
Microsoft Windows. It's safer to check the encoding.
...
# unzip, uchardet, and wc must be installed

# Get the character encoding
unzip -p data/VR_Snapshot_20081104.zip | uchardet
#> UTF-16

# Get the number of lines in the file (including header row)
unzip -p data/VR_Snapshot_20081104.zip | wc -l
#> 9671887
```

-   The data file is encoded in UTF-16

-   The data file contains 9,671,887 lines (including the header line)

While attempting to read the entity data file I discovered that about
half the apparently empty fields contained an embedded null character. I
don't know why, this is just how the data is provided. This can cause
[problems](https://github.com/r-lib/vroom/issues/340) for some ways to
read the data. There are ways to read this data in R, but I decided it
was more robust to re-encode the characters and delete the nulls before
reading it with R.

```{bash eval = FALSE}
# Preprocess the data file
# This probably only works on linux
# unzip, iconv, tr, and xz must be installed

unzip -p data/VR_Snapshot_20081104.zip |     # uncompress
iconv -f UTF-16LE -t UTF-8//TRANSLIT |       # convert encoding to UTF-8
tr -d '\000' |                               # delete nulls
xz --threads=0 >data/VR_Snapshot_20081104.xz # compress as XZ
