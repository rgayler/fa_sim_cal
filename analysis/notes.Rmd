...
-   Partition the entity data records uniformly and independently into
    ...
    -   The construction of all subsamples of the training set will be
        paralleled in the construction of subsamples of the testing set.
...
made less visually obtrusive, for clarity).
...
The "train/test 50:50 splitter" nodes represent the replication of the
train/test split as many times as are needed.

The "query sampler: n_q" node randomly samples $n_q$ records from the
"train universe" to be used as the "train queries" to be run against all
of the dictionaries sampled from the "train universe".

The "dictionary sampler: 90%" node randomly selects 90% of the "train
universe" records to use as a dictionary.
...
The construction of the data sets under each of the "train/test 50:50
splitter" nodes exactly parallels the construction under the "train/test
50:50 splitter 1" node.
