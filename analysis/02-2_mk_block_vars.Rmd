...
occur much later in the project.
...
-   There are 43 blocks with sizes in the "useful" range.
    ...
    -   The blocks corresponding to ages 18 to 63 inclusive are larger
        ...
        -   However, thee are all considerably less than 100k records,
...
# Modified multivariate blocking

Create modified sex age, and county ID variables for potential use in
(sex, age) and (sex, county) as blocking variables.

Exclude missing values to avoid the nuisance of having to deal with them
properly.

Pool the smaller age and county groups so that the smallest blocking
groups are not too small.

## Missing values

Identify the missing values and flag them as exclusions for blocking and
modelling.
...
d <- d %>% 
  dplyr::mutate(
    sex_miss = sex == "UNK",
    excl_blk_miss = sex_miss | age_cln_miss # county ID is never missing
  )

table(d$excl_blk_miss)
table(d$excl_blk_miss) %>% prop.table() %>% round(3)
```

This will exclude \~47k (\~1%) of records from blocking and modelling.

Look at the distributions of block sizes after excluding the records
with missing values in the blocking variables.

```{r}
# Calculate all the block sizes for each interesting combination of blocking variables.
# For only 6 combinations I will manually enumerate them.
x <- d %>% 
  dplyr::filter(!excl_blk_miss)

d_blk <- dplyr::bind_rows(
  list(
    sex_age_cnty = dplyr::count( x, sex, age_cln, county_id, name = "blk_sz"),
    sex_age      = dplyr::count( x, sex, age_cln           , name = "blk_sz"),
    sex_cnty     = dplyr::count( x, sex,          county_id, name = "blk_sz"),
    age_cnty     = dplyr::count( x,      age_cln, county_id, name = "blk_sz"),
    age          = dplyr::count( x,      age_cln           , name = "blk_sz"),
    cnty         = dplyr::count( x,               county_id, name = "blk_sz")
  ),
  .id = "blk_vars"
) %>% 
  dplyr::mutate(
...
-   It is clear that (sex, age) is the only combination worth
    considering and it wil need some pooling of the smallest age groups.
...
d %>% 
  dplyr::filter(!excl_blk_miss & 
                  (age_cln <= 18 | age_cln >= 95)) %>%
  dplyr::select(sex, age_cln) %>% 
  with(table(age_cln, sex))
```

-   Pool the 17 year olds with the 18 year olds

-   Pooling 102 and 103 would be adequate.

    -   However the spikes of records at 101 and 104 are somewhat
        suspect. So I will pool al the records with ages over 100 in
...
d <- d %>% 
  dplyr::mutate(
    age_blk = dplyr::case_when(
      excl_blk_miss  ~ NA_integer_, # make value NA for safety
    ...
    sex_age_blk = dplyr::if_else(excl_blk_miss,
                                 NA_character_, # make value NA for safety
                                 paste(sex, age_blk, sep = ":")
    )
  ) %>% 
  dplyr::select(-age_blk) # we won't use age alone as a blocking variable
...
d_blk <- d %>% 
  dplyr::filter(!excl_blk_miss) %>% 
  dplyr::count(sex_age_blk, name = "blk_sz")
...
Try pooling some of the highest ages to increase the smallest block
sizes above the lower limit of useful block size.
